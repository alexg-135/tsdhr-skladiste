...
<body>
    ...
    <?php
        ...
        elseif (isset($_POST["id"]) && isset($_POST["delete"])) {
            ...
            $sql = "DELETE FROM `vrstarobe` WHERE `ID` = ".$_POST["id"];
            if ($DB->query($sql) === TRUE) {
                ...
                $query = $DB->query("SELECT * FROM `vrstarobe` WHERE `ID` = ".$_POST["id"]);
    ...
    <div class="card m-4">
        ...
        <div class="card-body">
            <?php 
                if ($error == 1) {
                    print("<div class=\"alert alert-danger\" role=\"alert\"><h3>Vrsta se ne može izbrisati!</h3><hr><p>Provjerite je li postoji roba povezana na tu vrstu</p></div>");
                }
            ?>

            <form action="./vrsta-izbrisi.php" method="POST">
                ...
                <div class="input-group mb-3">
                    ...
                    <input disabled name="napomena" type="text" class="form-control" placeholder="Napomena" aria-label="Napomena" aria-describedby="basic-addon1" value="<?php print($vrsta["Napomena"]); ?>">
                </div>

                <input type="hidden" name="id" value="<?php print($vrsta["ID"]); ?>">
                <input type="hidden" name="delete" value="true">
                <button class="btn btn-primary">Izbriši zauvijek</button>
                <a class="btn btn-danger" href="./uredi-vrstu.php?id=<?php print($vrsta["ID"]); ?>&return=.%2Fvrsta-lista.php?page=1">Odustani</a>
            </form>
        </div>
    </div>
